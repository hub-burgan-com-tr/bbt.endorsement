...
namespace Application.Parameter.Queries.GetParameters
{
    ...
    public class GetParameterQueryHandler : IRequestHandler<GetParameterQuery, Response<List<GetParameterDto>>>
    {
        ...
        {
            var response = _context.Parameters.Where(x=>x.Text==request.Text).Select(x => new GetParameterDto { Text = x.Text, DmsReferenceId = x.DmsReferenceId,DmsReferenceKey=x.DmsReferenceKey }).OrderBy(x => x.Text).ToList();
