...
namespace Application.Endorsements.Queries.GetApprovalsFormDocumentDetail
{
    ...
    public class GetApprovalsFormDocumentDetailQueryHandler : IRequestHandler<GetApprovalFormDocumentDetailQuery, Response<GetApprovalFormDocumentDetailDto>>
    {
        ...
        {
            var response = _context.Documents.Where(x => x.OrderId == request.OrderId && x.Type == ContentType.HTML.ToString()).Select(x => new GetApprovalFormDocumentDetailDto {DocumentId=x.DocumentId,Title=x.Order.Title, Name = x.Name, CitizenShipNumber = "", FirstAndSurname = "", Actions = x.Actions.Select(x => new Action {ActionId=x.ActionId, IsDefault = x.IsDefault, Title = x.Title }).ToList() }).FirstOrDefault(); 
