...
namespace Application.Endorsements.Queries.GetApprovalsFormDocumentDetail
{
    ...
    public class GetApprovalsFormDocumentDetailQueryHandler : IRequestHandler<GetApprovalFormDocumentDetailQuery, Response<GetApprovalFormDocumentDetailDto>>
    {
        ...
        {
            var response = _context.Documents.Where(x => x.Order.OrderId == request.OrderId && x.Type == ContentType.HTML.ToString()).Select(x => new GetApprovalFormDocumentDetailDto { Name = x.Name, CitizenShipNumber = "", FirstAndSurname = "", Actions = x.Actions.Select(x => new Action { IsDefault = x.IsDefault, Title = x.Title }).ToList() }).FirstOrDefault(); 
