    ...
    public class GetMyApprovalDetailQueryHandler : IRequestHandler<GetMyApprovalDetailsQuery, Response<GetMyApprovalDetailsDto>>
    {
        ...
        public async Task<Response<GetMyApprovalDetailsDto>> Handle(GetMyApprovalDetailsQuery request, CancellationToken cancellationToken)
        {
            var response = _context.Orders
                .Where(x => x.State != OrderState.Pending.ToString() &&
                            x.State != OrderState.Cancel.ToString() &&
                            x.Customer.CitizenshipNumber == request.CitizenshipNumber)
                .Include(x => x.Documents)
                .ThenInclude(x => x.DocumentActions)
                .Include(x => x.Documents).ThenInclude(x => x.FormDefinition)
                .ThenInclude(x => x.FormDefinitionActions)
                .Include(x => x.OrderHistories).Where(x => x.OrderId == request.OrderId)
                .Select(x => new GetMyApprovalDetailsDto
                {
                    ...
                    Documents = x.Documents.OrderByDescending(x => x.Created).Select(x => new OrderDocument
                    {
                        ...
                        Type = x.FileType,
                        FileName = x.Name + ".pdf",
                        Actions = x.DocumentActions.Where(x => x.IsSelected).Select(y => new Action { Checked = y.IsSelected, Title = y.Title, DocumentId = x.DocumentId }).FirstOrDefault()
                    }).ToList(),
                    History = x.OrderHistories.Where(x => x.IsStaff)
                        .OrderByDescending(x => x.Created)
                        .Select(x =>
                            new GetMyApprovalDetailHistoryDto
                            {
                                CreatedDate = x.Created.ToString("dd.MM.yyyy HH:mm"),
