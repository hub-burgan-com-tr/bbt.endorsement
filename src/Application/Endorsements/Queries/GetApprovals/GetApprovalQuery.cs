...
namespace Application.Endorsements.Queries.GetApprovals
{
    ...
    public class GetApprovalQueryHandler : IRequestHandler<GetApprovalQuery, Response<PaginatedList<GetApprovalDto>>>
    {
        ...
        {
            var list = await _context.Orders.Include(x => x.Documents).OrderBy(x => x.Title).ThenByDescending(x => x.Created).Select(x => new GetApprovalDto { OrderId = x.OrderId, Title = x.Title, IsDocument = x.Documents.Any() }).PaginatedListAsync(request.PageNumber, request.PageSize);
