...
namespace Application.Endorsements.Queries.GetWantApprovalsDetails
{
    ...
    public class GetWantApprovalDetailQueryHandler : IRequestHandler<GetWantApprovalDetailsQuery, Response<GetWantApprovalDetailsDto>>
    {
        ...
        {
            var response = await _context.Orders.Include(x=>x.Customer).Include(x => x.Documents).ThenInclude(x=>x.DocumentActions).Include(x=>x.DocumentHistories).Where(x=>x.OrderId==request.OrderId).Select(x => new GetWantApprovalDetailsDto 
            { OrderId = x.OrderId,
                Title = x.Title,
                ...
                History = x.DocumentHistories.Select(x => new GetWantApprovalDetailsHistoryDto { CreatedDate = DateTime.Now.ToString("dd MM yyyy HH:mm"), Name = x.Name, State = x.State }).ToList(),
