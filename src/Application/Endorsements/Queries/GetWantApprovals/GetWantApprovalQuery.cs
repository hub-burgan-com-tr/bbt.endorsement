...
namespace Application.Endorsements.Queries.GetWantApprovals
{
    ...
    public class GetWantApprovalQueryHandler : IRequestHandler<GetWantApprovalQuery, Response<PaginatedList<GetWantApprovalDto>>>
    {
        ...
        {
            var list = await _context.Orders.Where(x=>x.Person.CitizenshipNumber == request.CitizenshipNumber).Include(x=>x.Documents).Include(x=>x.Customer).OrderByDescending(x => x.Created).Select(x => new GetWantApprovalDto { OrderId = x.OrderId, Title = x.Title, IsDocument = x.Documents.Any(x => x.Type != ContentType.PlainText.ToString() && x.FormDefinitionId == null),
                NameAndSurname=x.Reference.ProcessNo!=null?x.Customer.FirstName+" "+x.Customer.LastName+", "+x.Reference.ProcessNo: x.Customer.FirstName + " " + x.Customer.LastName,
