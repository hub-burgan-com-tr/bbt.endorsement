...
namespace Application.Endorsements.Commands.NewOrders
{
    public class NewOrderCommandValidator : AbstractValidator<StartRequest>
    {
        public NewOrderCommandValidator()
        {
            ...
            RuleFor(x => x.Documents.Any(x => x.Type.ToString() == ContentType.PDF.ToString())).NotNull().DependentRules(() =>
            {
                RuleFor(v => v.Documents.Any(x => x.Content == null)).Empty().WithMessage("Lütfen dosya seçiniz");
            });
            RuleFor(x => x.Documents.Any(x => x.Type.ToString() == ContentType.PlainText.ToString())).NotNull().DependentRules(() =>
            {
