...
namespace Application.Endorsements.Commands.NewOrders
{
    public class NewOrderCommandValidator : AbstractValidator<StartRequest>
    {
        public NewOrderCommandValidator()
        {
            ...
            //RuleFor(x => x.Documents.Any(x => x.Type == (int)DocumentTypeEnum.Physically)).NotNull().DependentRules(() =>
            //{
            //    RuleFor(v => v.Documents.Any(x => x.File == null)).Empty().WithMessage("Lütfen dosya seçiniz");
            //});
