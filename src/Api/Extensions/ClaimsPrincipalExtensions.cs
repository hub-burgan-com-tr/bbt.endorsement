...
public static class ClaimsPrincipalExtensions
{
    ...
    public static bool IsCredentials(this ClaimsPrincipal principal, string requestUserName)
    {
        ...
        try
        {
            // Check for existing credentials in the principal claims
            if (string.IsNullOrEmpty(requestUserName) || principal.Claims.Any(c => c.Type == "credentials" && c.Value.Contains("isReadyFormCreator")))
            {
                return principal.Claims.Any(c => c.Type == "credentials");
            }

            Log.Information("GetSSOClaims start  " + requestUserName);

            // Retrieve and map SSO claims asynchronously
        ...
        // Check if credentials exist in claims
        return principal.Claims.Any(c => c.Type == "credentials");
    }

    private static async Task<ClaimsPrincipal> GetSSOClaims(ClaimsPrincipal principal, string requestUserName)
    {
        if (string.IsNullOrEmpty(requestUserName) || requestUserName.Length < 4)
        {
            return principal;
        }

        var cacheKey = requestUserName;
        var ssoResponse = _cacheProvider.Contains(cacheKey)
            ? _cacheProvider.Get(cacheKey) as SSOIntegrationResponse
            : await FetchSSOClaims(requestUserName);

        if (ssoResponse != null)
        {
            return SSOResponseMapClaims(principal, ssoResponse);
        }

        return principal;
    }

    private static async Task<SSOIntegrationResponse> FetchSSOClaims(string requestUserName)
    {
        ...
        var ssoResponse = new SSOIntegrationResponse { RegisterId = responseRegisterId.Data };
        var resUserByRegisterId = await ssoService.GetUserByRegisterId(ssoResponse.RegisterId);
        ...
        ssoResponse.UserInfo = resUserByRegisterId.Data;
        var resAuthorityForUser = await ssoService.GetAuthorityForUser("MOBIL_ONAY", "Credentials", ssoResponse.UserInfo.LoginName);

        if (resAuthorityForUser.StatusCode == 200)
        {
            ssoResponse.UserAuthorities = resAuthorityForUser.Data;
            // Cache the response
            _cacheProvider.Set(requestUserName, ssoResponse, TimeSpan.FromMinutes(45));
        }
    ...
    private static ClaimsPrincipal SSOResponseMapClaims(ClaimsPrincipal principal, SSOIntegrationResponse ssoResponse)
    {
        ...
        // Remove any existing 'username' claim
        identity.RemoveClaim(identity.FindFirst(x => x.Type == "username"));

        // Add claims from SSO response
        ...
        // Add credentials based on authorities
        foreach (var authority in ssoResponse.UserAuthorities.Where(x => x.Name.StartsWith("is")))
        {
            identity.AddClaim(new Claim("credentials", $"{authority.Name}###{authority.Value}"));
        }
